...
/**
 * Supabase에 보고서 메타데이터 저장
 */
export async function notifyReportGenerated(metadata: ReportMetadata): Promise<void> {
  try {
    console.log(`[Supabase] 📢 Sending notification for report: ${metadata.report_path}`);

    // Supabase URL이 설정되어 있지 않으면 건너뛰기
    const supabaseUrl = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
    const supabaseKey = process.env.SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

    if (!supabaseUrl || !supabaseKey) {
      console.log(`[Supabase] ⏭️  Supabase not configured, skipping notification`);
      return;
    }

    // Supabase 클라이언트 초기화 (동적 import)
    ...
    if (error) {
      console.error(`[Supabase] ❌ Failed to insert report metadata:`, error);
      return;
    }

    console.log(`[Supabase] ✅ Notification sent successfully`);
...
async function sendWebhookNotification(metadata: ReportMetadata): Promise<void> {
  try {
    ...
    console.log(`[Webhook] 📢 Sending webhook notification...`);
...
export async function sendSlackNotification(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[Slack] ❌ Failed to send Slack notification: ${response.status}`);
      return;
    }
    ...
    console.error(`[Slack] ❌ Slack error:`, error);
  }
}
...
export async function sendDiscordNotification(
  ...
  try {
    ...
    console.log(`[Discord] 📢 Sending Discord notification...`);
    ...
    if (!response.ok) {
      console.error(`[Discord] ❌ Failed to send Discord notification: ${response.status}`);
      return;
    }

    console.log(`[Discord] ✅ Discord notification sent successfully`);

  } catch (error) {
    console.error(`[Discord] ❌ Discord error:`, error);
  }
}
